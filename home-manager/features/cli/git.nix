{ pkgs, ... }: {
  programs.git = {
    ...
    aliases = {
      ...
      create-branch =
        "!sh -c 'git push origin HEAD:refs/heads/$1 && git fetch origin && git branch --track $1 origin/$1 && cd . && git checkout $1' -";
      delete-branch =
        "!sh -c 'git push origin :refs/heads/$1 && git remote prune origin && git branch -D $1' -";
      ...
      nb =
        "checkout -b"; # create and switch to a new branch (mnemonic: "git new branch branchname...")
      ...
      llog = "log --date=local";
      lg =
        "log --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %Cgreen(%cr) %C(bold blue)<%an>%Creset %s' --abbrev-commit --date=relative";
      changes =
        ''log --pretty=format:"%h %cr %cn %Cgreen%s%Creset" --name-status'';
      ...
      uncommit =
        "reset --soft HEAD^"; # go back before last commit, with files in uncommitted state
      filelog = "log -u"; # show changes to a file
      mt = "mergetool"; # fire up the merge tool

      # serve the repo
      serve =
        "!git daemon --reuseaddr --verbose --base-path=. --export-all --informative-errors";
